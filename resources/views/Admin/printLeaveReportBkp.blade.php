...
<head>
    ...
    <style>
        @page {
            ...
            @bottom-center {
                content: "Employee Leave Management System - Confidential";
                font-size: 8pt;
                color: #9ca3af;
            }
        }
        ...
        * {
        ...
        @media print {
            ...
            /* Only break between cards if necessary */
            .leave-card {
                page-break-inside: avoid;
                break-inside: avoid;
            }

            /* Keep section title with at least first row of cards */
            .section-title {
                page-break-after: avoid;
            }
        ...
        /* Fallback for browsers that don't support @page margins */
        @supports not (content: counter(page)) {
            .page-number::after {
...
<body>
    <!-- Page Footer (for browsers that don't support @page) -->
    <div class="page-footer">
        Employee Leave Management System - Confidential Report
    </div>
    ...
    <div class="header">
        ...
        <div class="header-content">
            ...
            <div class="subtitle">Employee Leave Management System</div>
        </div>
    </div>
    ...
    <div class="report-info">
        ...
        <div>
            <strong>Department:</strong> {{ $departmentFilter ? $departmentFilter : 'All Departments' }}
        </div>
        <div>
            <strong>Generated:</strong> {{ date('F d, Y - H:i') }}
        </div>
        <div>
            <strong>Total Records:</strong> {{ $leaveRequests->count() }}
        </div>
    </div>
    ...
    <div class="statistics avoid-break">
        <div class="stat-card pending">
            <div class="number">{{ $leaveRequests->where('status', 'pending')->count() }}</div>
            <div class="label">Pending</div>
        </div>
        <div class="stat-card approved">
            <div class="number">{{ $leaveRequests->where('status', 'Approved')->count() }}</div>
            <div class="label">Approved</div>
        </div>
        <div class="stat-card rejected">
            <div class="number">{{ $leaveRequests->where('status', 'Rejected')->count() }}</div>
            <div class="label">Rejected</div>
        </div>
        <div class="stat-card total">
            <div class="number">{{ $leaveRequests->count() }}</div>
    ...
    @if($pendingRequests->count() > 0)
        <div class="section-wrapper">
            <div class="section-title">Pending Requests ({{ $pendingRequests->count() }})</div>
                    ...
                    <div class="leave-card pending">
                        ...
                        <div class="details">
                            ...
                            <div class="detail-row">
                                ...
                                <span class="detail-value">{{ \Carbon\Carbon::parse($request->leave_from)->diffInDays(\Carbon\Carbon::parse($request->leave_to)) + 1 }} days</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">From:</span>
                                <span class="detail-value">{{ \Carbon\Carbon::parse($request->leave_from)->format('M d, Y') }}</span>
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">To:</span>
                                <span class="detail-value">{{ \Carbon\Carbon::parse($request->leave_to)->format('M d, Y') }}</span>
                            </div>
                            @if($request->employee->department)
                                <div class="detail-row">
                                    <span class="detail-label">Department:</span>
                                    <span class="detail-value">{{ $request->employee->department }}</span>
                                </div>
                            @endif
                            @if($request->reason)
                                <div class="reason">
    ...
    @if($approvedRequests->count() > 0)
        <div class="section-wrapper">
            <div class="section-title {{ $pendingRequests->count() > 0 ? 'page-break' : '' }}">Approved Requests ({{ $approvedRequests->count() }})</div>
            <div class="leave-cards-grid">
    ...
    @if($rejectedRequests->count() > 0)
        <div class="section-wrapper">
            <div class="section-title {{ ($pendingRequests->count() > 0 || $approvedRequests->count() > 0) ? 'page-break' : '' }}">Rejected Requests ({{ $rejectedRequests->count() }})</div>
            <div class="leave-cards-grid">
    ...
    <!-- Footer -->
    <div class="footer">
        <p>Employee Leave Management System - Confidential Report</p>
        <p>This report contains {{ $leaveRequests->count() }} leave request(s) for {{ $yearFilter }}</p>
    </div>

    <script>
        // Simple page counter for browsers that don't support CSS counters in print
        let pageCounter = 1;
        window.addEventListener('beforeprint', function() {
            const counters = document.querySelectorAll('.page-counter');
            counters.forEach(counter => {
                counter.textContent = pageCounter;
            });
        });
